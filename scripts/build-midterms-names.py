#!/usr/bin/env python3
"""Draw each state's name for the week one share cards.

    pip install pillow
    python3 scripts/build-midterms-names.py

Writes assets/images/midterms/week1-names/<crop>/<code>.png for both crops,
one per entry in data/midterms_states.yml. Each is a transparent layer the
size of its card that layouts/partials/midterms-card-week1.html lays over the
plate at 0, 0.

The design stands the name on the plate's diagonal: Bebas Neue, stretched
tall, bottoms flat and tops running up along the diagonal, so every letter
tapers as well as each being taller than the last. Hugo can draw text but not
distort it, so the names are drawn here once and checked in. Re-run after
changing a state name or the layout numbers below. Temporary, like the rest
of /midterms.
"""
...
STATES = "data/midterms_states.yml"
FONT = "assets/fonts/BebasNeue-Regular.ttf"
OUT_DIR = "assets/images/midterms/week1-names"
