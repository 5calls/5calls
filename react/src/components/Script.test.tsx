...
describe('Script Component', () => {
  ...
  const renderScriptComponent = (locationState?: LocationState) => {
    return render(
      <ScriptTestWrapper
        ref={(instance: Script) => { scriptInstance = instance; }}
  ...
  describe('scriptFormat function', () => {
    ...
    describe('with undefined locationState', () => {
      it('should not replace location placeholders when locationState is undefined', () => {
        const script = 'Hello [REP/SEN NAME], I am calling from [CITY, ZIP] to discuss this issue.';
        const result = scriptInstance!.scriptFormat(script, undefined, mockContact);
        
        expect(result).toBe('Hello Rep. John Smith, I am calling from [CITY, ZIP] to discuss this issue.');
      ...
      it('should not replace location placeholders when locationState has no cachedCity', () => {
        const locationStateWithoutCity = { ...mockLocationState, cachedCity: '' };
        const script = 'Hello [REP/SEN NAME], I am calling from [CITY, STATE] to discuss this issue.';
        const result = scriptInstance!.scriptFormat(script, locationStateWithoutCity, mockContact);
        
        expect(result).toBe('Hello Rep. John Smith, I am calling from [CITY, STATE] to discuss this issue.');
      ...
      it('should handle both location placeholder formats with undefined locationState', () => {
        const script = 'I live in [CITY, ZIP] and work in [CITY, STATE]. Please help [REP/SEN NAME].';
        const result = scriptInstance!.scriptFormat(script, undefined, mockContact);
        
        expect(result).toBe('I live in [CITY, ZIP] and work in [CITY, STATE]. Please help Rep. John Smith.');
    ...
    describe('with undefined contact', () => {
      it('should not replace contact name placeholders when contact is undefined', () => {
        const script = 'Hello [REP/SEN NAME], I am calling from [CITY, ZIP] to discuss this issue.';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, undefined);
        
        expect(result).toBe('Hello [REP/SEN NAME], I am calling from Springfield, IL to discuss this issue.');
      ...
      it('should handle both contact placeholder formats with undefined contact', () => {
        const script = 'Dear [REP/SEN NAME], also known as [SENATOR/REP NAME], I need your help.';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, undefined);
        
        expect(result).toBe('Dear [REP/SEN NAME], also known as [SENATOR/REP NAME], I need your help.');
      ...
      it('should still replace location placeholders when contact is undefined', () => {
        const script = 'I am calling from [CITY, ZIP] to ask [REP/SEN NAME] about this issue.';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, undefined);
        
        expect(result).toBe('I am calling from Springfield, IL to ask [REP/SEN NAME] about this issue.');
    ...
    describe('with both undefined', () => {
      it('should not replace any placeholders when both locationState and contact are undefined', () => {
        const script = 'Hello [REP/SEN NAME], I am calling from [CITY, ZIP] to discuss this issue.';
        const result = scriptInstance!.scriptFormat(script, undefined, undefined);
        
        expect(result).toBe('Hello [REP/SEN NAME], I am calling from [CITY, ZIP] to discuss this issue.');
        expect(result).toContain('[REP/SEN NAME]');
        expect(result).toContain('[CITY, ZIP]');
      });

      it('should return the original script unchanged when no replacements are possible', () => {
        const script = 'This is a script with [REP/SEN NAME] and [CITY, STATE] and [SENATOR/REP NAME] and [CITY, ZIP].';
        const result = scriptInstance!.scriptFormat(script, undefined, undefined);
    ...
    describe('with valid inputs', () => {
      it('should replace both location and contact placeholders correctly', () => {
        const script = 'Hello [REP/SEN NAME], I am calling from [CITY, ZIP] to discuss this issue.';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, mockContact);
        
        expect(result).toBe('Hello Rep. John Smith, I am calling from Springfield, IL to discuss this issue.');
      ...
      it('should handle case-insensitive replacement for contact placeholders', () => {
        const script = 'Hello [rep/sen name], please help [REP/SEN NAME] and [Senator/Rep Name].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, mockContact);
        
        expect(result).toBe('Hello Rep. John Smith, please help Rep. John Smith and Rep. John Smith.');
      });
      ...
      it('should handle case-insensitive replacement for location placeholders', () => {
        ...
        expect(result).toBe('I live in Springfield, IL and work in Springfield, IL.');
      });

      it('should handle multiple instances of the same placeholder', () => {
        const script = '[REP/SEN NAME] please help. Thank you [REP/SEN NAME]. From [CITY, ZIP] to [CITY, ZIP].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, mockContact);
        
        expect(result).toBe('Rep. John Smith please help. Thank you Rep. John Smith. From Springfield, IL to Springfield, IL.');
      });
    });
    ...
    describe('contact title formatting', () => {
      ...
      it('should format Senate representative correctly', () => {
        const script = 'Hello [REP/SEN NAME].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, mockSenatorContact);
      ...
      it('should format Governor correctly', () => {
        const script = 'Hello [REP/SEN NAME].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, mockGovernorContact);
      ...
      it('should handle StateLower area', () => {
        ...
        const script = 'Hello [REP/SEN NAME].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, stateLowerContact);
      ...
      it('should handle StateUpper area', () => {
        ...
        const script = 'Hello [REP/SEN NAME].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, stateUpperContact);
      ...
      it('should handle AttorneysGeneral area', () => {
        ...
        const script = 'Hello [REP/SEN NAME].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, agContact);
      ...
      it('should handle SecretaryOfState area', () => {
        ...
        const script = 'Hello [REP/SEN NAME].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, sosContact);
      ...
      it('should handle unknown area with no title', () => {
        ...
        const script = 'Hello [REP/SEN NAME].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, unknownContact);
      ...
      it('should handle contact with no area', () => {
        ...
        const script = 'Hello [REP/SEN NAME].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, noAreaContact);
      ...
      it('should handle alternative area names', () => {
        ...
        const houseResult = scriptInstance!.scriptFormat(script, mockLocationState, houseContact);
        const senateResult = scriptInstance!.scriptFormat(script, mockLocationState, senateContact);
    ...
    describe('edge cases', () => {
      ...
      it('should handle script with no placeholders', () => {
        ...
        expect(result).toBe('This is a regular script with no special placeholders.');
      });
      ...
      it('should handle malformed placeholders', () => {
        const script = 'Hello [REP/SEN] and [CITY] and [NAME] - these should not be replaced.';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, mockContact);
        
        expect(result).toBe('Hello [REP/SEN] and [CITY] and [NAME] - these should not be replaced.');
      });

      it('should handle placeholders with optional whitespace', () => {
        const script = 'Hello [REP/SEN NAME] and from [CITY,ZIP] and [CITY, STATE].';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, mockContact);
        
        // The regex handles 0 or 1 whitespace after comma
        expect(result).toBe('Hello Rep. John Smith and from Springfield, IL and Springfield, IL.');
      });

      it('should not replace placeholders with extra whitespace beyond what regex supports', () => {
        const script = 'Hello [REP/SEN NAME] and from [CITY,  ZIP] with 2 spaces.';
        const result = scriptInstance!.scriptFormat(script, mockLocationState, mockContact);
        
        // The regex only handles 0 or 1 whitespace, so 2 spaces won't match
        expect(result).toBe('Hello Rep. John Smith and from [CITY,  ZIP] with 2 spaces.');
